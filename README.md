...
- 《[图像物理内涵说明](图像物理内涵说明.md)》——汇总各阶段数值图像的
  物理内涵、判读要点与解析参考值（图像嵌入，供论文插图对照）。
- 《[各阶段分析](stage%20analysis/README.md)》——按阶段拆分（stage0.md–stage12.md），
  各阶段具体内容、涉及文件与关键分析要点（含 Stage 8 FDM vs PINN 界面处理差异）。
- 《Hybrid SP-PINN 一维平衡态 MOS 电容项目搭建说明》（项目根目录上一级）
