...
## 8.4 无量纲化修正（原始方案 §3.5 有误，须修正）

- 原始方案用本征德拜长度 `L_D=√(ε_si·kT/(q²·n_i))`，代入得 **L_D≈33 μm**，而器件仅 102 nm，`z̄∈[0,0.003]`，输入被压缩到近 0，是灾难性尺度。
...
1. **论文式 (3.2) 损失 = 全域 Laplacian 形式 `d²φ̄/dz̄²+ρ̄`——物理/量纲错误**。ρ̄ 只做浓度归一，ε 与 L² 因子整个丢失，等于假设 ε 均匀（把 3 倍介电跳变丢掉，同 §8.3 与原始方案 §4.3 的错误）。代码采用解析氧化层 + Robin（正确）。**论文第三章须改写**。
...
8. **Stage 12 参数化**：Vg 并入网络输入后，Robin 损失显式依赖 Vg，需把 Vg 按批并入 `_loss`——架构兼容，非障碍。（**2026-08-19 更正**：此「非障碍」仅指 Vg 批处理这一机械层面；「跳过自洽循环」的真正障碍在 Schrödinger 不可微，Stage 12 已重定位为**有监督参数化神经代理**，见 [stage12.md](stage12.md) §12.2。）
